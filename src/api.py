...
class RealTimeAPI:
    ...
    def fetch_predictions(self, routes: list = routes) -> list:
        ...
        results = [
            result for result in results if result.get("arrival", None)
        ]  # filter out empty updates
        results = sorted(
            results,
            key=lambda x: int(x["arrival"].split(",")[0].strip(config["live_char"])),
        )  # sort by the next arrival for each route/dir/stop
        return results

    def fetch_route(
        self, route_name: str, route_color: int, route_params: dict, route_cutoff: int
    ) -> dict:
        ...
        while True:
            try:
                ...
                if r.status_code == 200:
                    ...
                    crossings = update["routeStops"][0]["stops"][0][
                        "crossings"
                    ]  # next two upcoming trips, can link to config variable
                    ...
                    for trip in crossings:
                        ...
                        pred_hour, pred_min = map(
                            int, trip[time_key + "Time"].split(":")
                        )
                        pred_ampm = trip[time_key + "Period"]
                        pred_hour = self.convert24(
                            pred_ampm, pred_hour
                        )  # convert to 24
                        ...
                        if (
                            pred_ampm == "am" and now_ampm == "pm"
                        ):  # if bus comes next day after midnight
                            arrival_in = (
                                (seconds_1day - now_secs) + pred_secs
                            ) // 60  # add remaining time to prediction time as minutes
                        else:
                            arrival_in = (
                                pred_secs - now_secs
                            ) // 60  # otherwise subtract prediction from current time as minutes
                        arrival_in = str(arrival_in - 1)
                        if time_key == "sched" and config["not_live_flag"]:
                            arrival_in += config[
                                "live_char"
                            ]  # add flag character when update is static and live GPS based
                        predictions.append(arrival_in)

                    # filter out all before our cutoff time of choice
                    predictions = list(
                        filter(
                            lambda x: int(x.strip(config["live_char"])) >= route_cutoff,
                            predictions,
                        )
                    )
                    # grab next 2 departs
                    ...
                    # Process destination info if provided. After Vontas OnRoute update, GCRTA's API doesn't broadcast destination info yet ("Tower City, Westgate")
                    if destination:
                        tokens = destination.split(" ")
                        if tokens[0] in ["Red", "Green", "Blue"]:
                            rail_color = tokens[0]
                            destination = destination.split(f"{rail_color} Line")[
                                1
                            ].strip()
                        else:
                    ...
                    return {
                        ...
                        "destination": "",  # output 3, hard-coded to be empty until destination returns
